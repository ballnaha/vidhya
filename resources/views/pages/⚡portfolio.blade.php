...
<main class="bg-[#0a0a0c] text-white" x-data="{ showModal: false, modalImage: '', modalTitle: '' }" @keydown.escape.window="showModal = false">
    <!-- Hero Section -->
    ...
    <section class="px-6 py-12 sm:px-10 lg:px-20 bg-black">
        <div class="mx-auto max-w-[1800px] space-y-12">
            ...
            <div class="border-b border-white/5 pb-10" data-reveal>
                <!-- Category Filters -->
                <div class="space-y-3">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#e60012]">Filter by Expertise</p>
                    <div class="flex flex-wrap gap-2">
                        <button 
                            type="button" 
                            wire:click="selectCategory('')" 
                            class="rounded border px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.12em] transition {{ $selectedCategory === '' ? 'border-[#366bc3] bg-white/8 text-white' : 'border-white/10 text-white/42 hover:border-white/25 hover:text-white' }}"
                        >
                            All Expertises
                        </button>
                        @foreach ($categories as $cat)
                            <button 
                                ...
                                class="rounded border px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.12em] transition {{ $selectedCategory === $cat ? 'border-[#366bc3] bg-white/8 text-white' : 'border-white/10 text-white/42 hover:border-white/25 hover:text-white' }}"
                            >
                                {{ $cat }}
                            </button>
                        @endforeach
                    ...
                    @foreach ($works as $index => $work)
                        ...
                        >
                            <button
                                ...
                                @click="modalImage = '{{ $work->image }}'; modalTitle = '{{ $work->title }} (by {{ $work->director_name }})'; showModal = true"
                            >
                                ...
                                <span class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 grid size-12 place-items-center rounded-full bg-[#366bc3]/85 text-white shadow-lg shadow-black/30 opacity-0 scale-90 transition-all duration-300 group-hover:opacity-100 group-hover:scale-100" aria-hidden="true">
            ...
            <!-- Contact CTA -->
            <div class="mt-24 text-center border-t border-white/5 pt-16" data-reveal>
                <p class="mb-4 text-xs font-semibold uppercase tracking-[0.26em] text-white/35">Ready to Scale Your Vision?</p>
                <h2 class="mb-8 text-2xl sm:text-3xl font-black uppercase tracking-tight text-white">Let's create something extraordinary together</h2>
                <a href="{{ route('contact') }}"
                    class="inline-flex rounded px-14 py-4 text-sm font-bold uppercase tracking-[0.1em] transition hover:brightness-110"
                    style="background: linear-gradient(90deg, #366bc3, #823665, #e60012);" wire:navigate.hover>Start a Project</a>
            </div>
        </div>
    </section>

    <!-- Global Premium Image Lightbox Modal -->
    <div 
    ...
    >
        ...
        <div class="relative max-w-[1200px] flex flex-col items-center gap-4" @click.stop>
            <!-- Large Image Wrapper -->
            <div class="relative overflow-hidden bg-black shadow-2xl border border-white/10 rounded-lg max-h-[80vh]">
